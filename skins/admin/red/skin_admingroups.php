		...
		<input type="hidden" value="{$obj->getId()}" name="{$this->modelName}[id]"/>
			<table class="obj_add_edit">
				...
				<tr>
					...
					<td>
					<label>
						<input <if="$obj->getDefault()=='1'">checked='checked'</if>  name="{$this->modelName}[default]" id="{$this->modelName}_default" type="checkbox" value="1"  />
					</label>
					</td>
				</tr>
				</if>
					...
					<foreach="$option['permission_list'] as $module => $list">
						...
						<h3>{$this->getLang()->getWords($module)}</h3>
							<foreach="$list as $key => $permission">
								<label><input <if="$option['myspermission'][$key]">checked='checked'</if> name="permission[$key]" type="checkbox" value="$key"/> $permission </label>
							</foreach>
						</if>
					</foreach>
					</if>
