	...
	function addEditObjForm($obj, $option = array()) {
		...
		<!--<input type="hidden" value="{$obj->getSlug ()}" name="{$this->modelName}[mUrl]" id="mUrl" data-module="{$this->modelName}" data-id = "{$obj->getId()}" />-->
			<table class="obj_add_edit" width="100%">
				...
				<tr>
					<td style="width: 121px;"><label>Trình trạng</label></td>
					<td>
					<label>
						<input <if="$obj->getState()=='0'">checked='checked'</if>  name="{$this->modelName}[state]" id="{$this->modelName}_state_0" type="radio" value="0"  />
						Hết hàng
					</label>
					<label>
						<input <if="$obj->getState()==1">checked='checked'</if>  name="{$this->modelName}[state]" id="{$this->modelName}_state_1" type="radio" value="1"  />
						Còn hàng
					</label>
					<label>
						<input <if="$obj->getState()==2">checked='checked'</if>  name="{$this->modelName}[state]" id="{$this->modelName}_state_1" type="radio" value="2"  />
						Bán chạy
					</label>
