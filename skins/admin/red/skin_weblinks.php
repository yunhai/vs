	...
	function addEditObjForm($objItem, $option = array(),$access) {
		...
		$BWHTML .= <<<EOF
			...
			<form id='add-edit-obj-form' name="add-edit-obj-form" method="POST"  enctype='multipart/form-data'>
				<input type="hidden" id="obj-cat-id" name="weblinkCatId" value="{$option['categoryId']}" />
				...
				<div class='ui-dialog ui-widget ui-widget-content ui-corner-all'>
					...
					<table class="ui-dialog-content ui-widget-content">
						...
						<tr>
							...
							<td>
								<input class='c_noneWidth' type="checkbox" checked name="weblinkStatus" id="weblinkStatus" {$objItem->getStatus()} value="1" style='margin-right:10px;'/>
								
								<if="$vsSettings->getSystemKey($bw->input[0].'_exptime',1)">
								{$vsLang->getWords('obj_exptime', 'Expire Time')}:&nbsp;&nbsp;<input size="4" type="text" name="weblinkExpTime" value="{$objItem->getExpTime()}" id="obj-exptime"/>
								</if>
							</td>
							</if>
						</tr>
						<tr>
							...
							<td>
								<if="$vsSettings->getSystemKey($bw->input[0].'_index',1)">
									<input size="4" type="text" name="weblinkIndex" value="{$objItem->getIndex()}" id="obj-Index"/>
									&nbsp; &nbsp; &nbsp; &nbsp;&nbsp;
								</if>
								
								<if="$vsSettings->getSystemKey($bw->input[0].'_price',1)">
								{$vsLang->getWords('obj_price', 'Price')}:&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<input size="14" type="text" name="weblinkPrice" value="{$objItem->getPrice()}" id="obj-price"/>
								</if>
							</td>
						</tr>
						<tr >
							...
							<td colspan="1">
								<div style="padding:2px 5px;">
									<input size="27" type="file" name="weblinkIntroImage" id="weblinkIntroImage"/> 
								</div>
							</td>
							</if>
						</tr>
						...
						<tr>
							...
							<td colspan="2">
								<if="$access">
									...
									<select style="width:250px;" id="weblinkAccess" name="relAccess">
			...
			<script language="javascript">
				...
				$(window).ready(function() {
					...
					<foreach="$option['access'] as $array">
						vsf.jSelect('{$array['relId']}','weblinkAccess');
					</foreach>
					</if>
					vsf.jCheckbox('{$objItem->getStatus()}','weblinkStatus');
				...
				$('#weblinkIntroImage').change(function() {
