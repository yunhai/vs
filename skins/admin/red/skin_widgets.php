					...
					<ul class="widget_list">
						<foreach="$block as $item">
								...
								<input type="hidden" value="$blockname" name="blockname"/>
									<div class="widget_block_item_bar"><h4 class="widget_item_click"><if="$item->getConfig()->title">{$this->cut($item->getConfig()->title,25)}<else />{$item->getObj()->getTitle()}</if></h4></div>
				...
				<tr>
					...
					<select name="{$this->modelName}[instant]" id="">
						<foreach="$option['widget_list'] as $index => $instant">
							<option <if="$obj->getInstant()==$index">selected='selected'</if> value="{$index}" >{$instant->getTitle()}</option>
						</foreach>
					</select>
					</td>
				</tr>
