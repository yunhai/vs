...
///define( 'WATERMARK_OVERLAY_IMAGE', 'styles/watermark/watermark.gif' );
...
//$watermark = (bool) get_request ('wm', 1);
...
$pos =1;
       if ($new_width > 150 && $new_height >150){
			$padding 		= 1; // distance to border in pixels for watermark image
			$opacity		= 100;	// image opacity for transparent watermark

			$watermark 	= imagecreatefrompng($watermark_img); // create watermark

			if(!$watermark) die("Error: main image or watermark could not be loaded!");

			$watermark_width = imagesx( $watermark );
			$watermark_height = imagesy( $watermark );  

			$dest_x 		= $new_width - $watermark_width - $padding;  
			$dest_y 		= $new_height - $watermark_height - $padding;

			// copy watermark on main image
			imagecopymerge($canvas, $watermark, $dest_x, $dest_y, 0, 0, $watermark_width, $watermark_height, $opacity);
