...
class PostParser {
	...
	function parse_album($html){
		return preg_replace_callback('#\<album\>(.+)\<\/album\>#', array("PostParser","parse_album_callback"), $html);
		
	}
	static function parse_album_callback($macth){
		$doc=new DOMDocument();
		@$doc->loadHTML($macth[0]);
		require_once CORE_PATH.'albums/albums.php';
		$albums=new albums();
		$albums->getObjectById($doc->getElementsByTagName("id")->item(0)->nodeValue);
		return $albums->getHtmlTemplate($albums->basicObject);
