...
class partners_admin {
	...
	function auto_run() {
		...
		switch($bw->input[1]){
			...
			case 'visible-checked-obj':
				$this->module->updateStatus(rtrim($bw->input['checkedObj'],","),array("partnerStatus" => 1));
				break;
			case 'hide-checked-obj':
				$this->module->updateStatus(rtrim($bw->input['checkedObj'],","),array("partnerStatus" => 0));
				break;
	...
	function getObjList($catId='', $message=""){
		...
		if(!intval($catId)){
			...
			$strIds = trim($catId.",".$this->module->vsMenu->getChildrenIdInTree($categories),",");
		}
	...
	function addEditObjForm($objId=0, $option=array()){
		...
		$editorHtml=$editor->createEditor("partnerContent",array('width'=>'650px','height'=>'350px'));
		$obj->setContent($editorHtml);
		$editor->setToolbarSet("narrow");
		$editor->setValue($obj->getIntro());
		$editorHtml=$editor->createEditor("partnerIntro",array('width'=>'515px;','height'=>'150px'));
	...
	function addEditObjProcess(){
		...
		if($bw->input['partnerStatus']) $bw->input['partnerStatus'] 	= 1;
		...
		$bw->input['partnerPosition'] = $posStr;
