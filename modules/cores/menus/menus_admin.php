...
class menus_admin 
{
	...
	function addEditCategoryProcess() {
		...
		if($bw->input['categoryId']){
			$categoryObj=$vsMenu->arrayCategory[$bw->input['categoryId']];
			$categoryObj->convertToObject($bw->input);
		}
		...
		if($bw->input['categoryRight']){
			$categoryObj->right=1;
		}
		if($bw->input['categoryToArea']){
			$categoryObj->setType(3);
		}
		if($bw->input['categoryFromArea']){
			$categoryObj->setIsLink(3);
		}
		...
		if($vsSettings->getSystemKey($bw->input['categoryGroup'].'_value',1)){
			...
			if($bw->input['hotelValueCat']) $categoryObj->setIsLink(isset($bw->input['categoryValue']));
		}
	...
	function addEditCategoryForm($categoryId=0, $option=array()) {
		global $vsLang,$bw,$vsSettings,$vsf;
		$vsSettings->getSystemKey($option['cate'].'_title',	1,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_status',1,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_intro',	0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_dropDown',0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_index',1,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_file',0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_value',0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_area_home',0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_right',0,$option['cate']);
		$vsSettings->getSystemKey($option['cate'].'_alias',0,$option['cate'],"Using alias Url");
		$categoryObj = new Menu();
		...
		if($categoryId&&$categoryId > 0){
			...
			if($vsSettings->getSystemKey($option['cate'].'_cate_show_seo',0,$option['cate'])){
				$flag=true;
				$categoryObj->parentAlias="";
				if($categoryObj->getLevel()>1){
					$parent=clone $this->module->arrayCategory[$categoryObj->getParentId()];
					while($parent->getLevel()>1){
						$realurl=$vsSettings->getSystemKey($option['cate']."_category_url",$option['cate']."/category/",$option['cate']);
						if(!$realurl){
							$realurl=$option['cate']."/category/";
						}
						$realurl.=$parent->getId();
						$aliasUrl=$vsf->getSeo()->getAliasByUrl($realurl);
						if($aliasUrl!=$realurl){//parent not seo
							$categoryObj->parentAlias=$aliasUrl.$categoryObj->parentAlias;
							$flag=false;
							break;
						}else{
							$categoryObj->parentAlias=
							str_replace(array("\"","'"),"", $parent->getTitle())."/".$categoryObj->parentAlias;
							$parent=clone $this->module->arrayCategory[$parent->getParentId()];
						}
					}
					if($flag){
						$categoryObj->parentAlias=$vsf->getSeo()->getAliasByUrl($option['cate']."/").$categoryObj->parentAlias;
					}
					if(!preg_match("/.+\/$/",$categoryObj->parentAlias)){
						$categoryObj->parentAlias.="/";
					}
	...
	function getCategoryBox($categoryGroup, $message="", $option=array()) {
		...
		if(!$option['id']) $option['id'] = 'menus-category'.$categoryGroup->getUrl();
		if(!$option['size']) $option['size'] = 18;
		if(!$option['multiple']) $option['multiple'] = true;
		if(!$option['onclick']) $option['onclick'] = "setValue_category{$categoryGroup->getUrl()}";
	...
	function displayMenuList() {
		...
		$menus = $vsMenu->filterMenu($filterArray,$vsMenu->arrayTreeMenu);
