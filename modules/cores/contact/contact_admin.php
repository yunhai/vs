...
require_once (CORE_PATH . "contact/contacts.php");
...
class contact_admin  {
	...
	function showContactList($contactType=0) {
		...
		$option = $this->module->getPageList("contact/showContact/{$contactType}", 3, $size, 1, 'ContactList');
	...
	function readContactProcess($contactId){
		...
		$contact->setStatus(1);
		$this->module->updateObjectById($contact);
	}
