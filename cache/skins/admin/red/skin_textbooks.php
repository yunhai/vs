...
//===========================================================================
// <vsf:displayBookTab:desc::trigger:>
//===========================================================================
function displayBookTab($option=array()) {global $vsLang, $bw;
...
{$this->__foreach_loop__id_4ff2728f1f213($option)}
...
vsf.get('textbooks/delete/'+jsonStr+'/'
EOF;
if( $option['textbooktype'] ) {
$BWHTML .= <<<EOF
+'&type={$option['textbooktype']}'
EOF;
}

$BWHTML .= <<<EOF
,'mainContainer{$option['textbooktype']}');
}
});
}
function editObj(id){
vsf.get('textbooks/editForm/'+id 
EOF;
if( $option['textbooktype'] ) {
$BWHTML .= <<<EOF
+'&type={$option['textbooktype']}'
...
//===========================================================================
function __foreach_loop__id_4ff2728f1f213($option=array())
{
global $vsLang, $bw;
    $BWHTML = '';
    $vsf_count = 0;
...
//===========================================================================
function editForm($obj="",$option="") {global $vsLang;
