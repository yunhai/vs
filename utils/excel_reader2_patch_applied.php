...
//nhan vao array name Obj in database and return array
	function getExcelData($arr = array(),$sheet = 0){
		$totacol = $this->sheets[$sheet]['numCols'];
		$datare=array();
		$definecol = count($arr);
		$dump =  $this->getFistIdCol($sheet);
		
		foreach ($this->sheets[$sheet]['cells'] as $key => $obj)
			for($i =0;$i<$definecol;$i++)
				$datare[] = $this->val($key,$i+$dump,$sheet);;
				$datare[$key][$arr[$i]]=$this->val($key,$i+$dump,$sheet);
		return 	$datare;
	}
	
	function getRawExcelData($sheet = 0){
		return $this->sheets[$sheet]['cells'];
	}
	
	function getFistIdCol($sheet=0){
		foreach ($this->sheets[$sheet]['cells'] as $key => $obj)
			foreach ($obj as $k => $o)
			return $k;
	}
	...
	function style($row,$col,$sheet=0,$properties='') {
		...
		// Only output border colors if there is an actual border cified
